...
class MusicAccessory {
  ...
  constructor (platform, id, name, on, volume, track) {
  ...
  update (on, volume, track) {
    ...
    if (track === '' && this.platform.resetTrack) {
      track = this.platform.track
    }
  ...
  // Called by homebridge when characteristic is changed from homekit.
  setChangeTrack (change, callback) {
