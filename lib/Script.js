...
class Script {
  constructor (platform, lib) {
    this.log = platform.log.bind(platform)
    this.warn = platform.warn.bind(platform)
    this.debug = platform.debug.bind(platform)
    this.timeout = platform.config.timeout * 1000
  ...
  async runScript (s, validate) {
    return new Promise((resolve, reject) => {
      ...
      const timeout = setTimeout(() => {
        timedOut = true
        this.warn('applescript %s: timeout', s)
        this.warn('            command: %s', script)
        return resolve()
      }, this.timeout)
      this.debug('applescript: %s', s)
      applescript.execString(script, (error, result) => {
        ...
        if (error) {
          this.warn('applescript %s: %s', s, error.message)
          this.warn('            command: %s', script)
          return resolve()
        }
        if (!result) {
          this.debug('applescript: %s => %j', s, result)
          return resolve()
        }
        let obj
        try {
          ...
          this.warn('applescript %s: invalid json: %s', s, error.message)
          this.warn('            command: %s', script)
          this.warn('            result: "%s"', result)
          return resolve()
        }
        if (!validate(obj, { greedy: true })) {
          this.warn('applescript %s: invalid result %s', s, validate.errors)
          this.warn('            command: %s', script)
          this.warn('            result: %j', obj)
          return resolve()
        }
        this.debug('applescript: %s => %j', s, obj)
        resolve(obj)
      })
    })
  }
