...
//   getMusicState(initial)     Gets player and speaker states (schema: GetState).
...
Script.prototype.runScript = function (s, validate) {
  return new Promise((resolve, reject) => {
    ...
    applescript.execString(script, (error, result) => {
      ...
      if (error) {
        this.log.error('applescript error %s', error.message)
        this.log.error('applescript: %s', script)
        return resolve()
      }
      let obj
      try {
        ...
        this.log.error('applescript returned invalid json (%s)', error.message)
        this.log.error('applescript: %s', script)
