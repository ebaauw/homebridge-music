...
Script.prototype.getInitialState = function (callback) {
  this.runScript('getInitialState()', validateGetState, callback)
}
...
Script.prototype.setOn = function (id, on, track, callback) {
  let s
  if (id) {
    s = util.format('setSpeakerOn("%s", %s, "%s")', id, on, this.location)
  } else {
    s = util.format('setPlayerOn(%s, "%s", "%s")', on, track, this.location)
  }
